...
<el-form-item label="提示数量:"
                                  class="collstat-form-item">
                      <div style="display: flex;">
                        <el-input v-model.number="notifyNumber"
                                style="width: 100%;"
                                type="number"
                                placeholder="请输入提示数量" />
                       <el-checkbox v-model="notifyFlag">是否提示</el-checkbox>
                      </div>
                    ...
                    <el-form-item :label="$t('CollectProducts._42')"
                                  class="collstat-form-item">
                      <el-input v-model="Carton_NO"
                                style="width: 100%;"
                                class="changText"
                                disabled
                                placfeholder=" " />
                    </el-form-item>
                    <el-form-item :label="$t('CollectProducts._43')"
                                  class="collstat-form-item">
                      <el-input v-model="DefinedQty"
                                type="Number"
                                style="width: 100%;"
                                placeholder=" " />
                    </el-form-item>
                    <el-form-item class="collstat-form-item"
                                  :label="$t('CollectProducts._44')">
                      <div style="display: flex;">
                        <el-input v-model="CurrentQty"
                                  class="changText"
                                  disabled
                                  placeholder=" " />
                        <el-button type="primary"
                                   @click="FullClick">{{$t('CollectProducts._45')}}</el-button>
                        <el-button type="primary"
                                   @click="ModifiesClick">{{$t('CollectProducts._59')}}</el-button>
                      </div>
                    </el-form-item>
                  </el-form>
                </div>
    ...
    <!-- 修改 -->
    <el-dialog class="my-dialog updateAuto"
               v-dialogDrag
               :title="$t('CollectProducts._60')"
               :visible.sync="ModifiesoShow"
               width="80%"
               :close-on-click-modal="false">
      <el-form ref="ModifiesRef"
               label-width="100px"
               :model="NewEdition">
        <el-form-item :label="$t('CollectProducts._61')+'：'"
                      style="width:50%">
          <el-select v-model="NewEdition.SiteId"
                     disabled
                     :placeholder="$t('CollectProducts._36')"
                     style="width: 100%">
        ...
        <el-form-item :label="$t('SfcsContainerList._1')+'：'"
                      style="width:50%">
          <div style="display: flex;">
            <el-input v-model="NewEdition.SN"
                      clearable
                      @keyup.enter.native="DataChange"
                      type="text"
                      placeholder=" " />
            <el-button type="primary"
                       @click="Clear">{{$t('SfcsProductPallet.clean')}}</el-button>
          </div>
        </el-form-item>
        <el-form-item :label="$t('SfcsContainerList._4')+'：'"
                      style="width:50%">
          <div style="display: flex;">
            <el-input v-model="NewEdition.CartonNo"
                      type="text"
                      disabled
                      placeholder=" " />
            <el-button type="primary"
                       @click="ModifiesFull">{{$t('CollectProducts._45')}}</el-button>
          </div>
        </el-form-item>
        <el-form-item :label="$t('SfcsContainerList._3')+'：'"
                      style="width:50%">
          <el-input v-model="NewEdition.DEFINEDQTY"
                    type="number"
                    disabled
                    placeholder=" " />
        </el-form-item>
        <el-form-item :label="$t('SfcsContainerList._5')+'：'"
                      style="width:50%">
          <el-input v-model="NewEdition.CURRENTQTY"
                    type="number"
                    disabled
                    placeholder=" " />
        </el-form-item>
      ...
      <div style="margin-bottom: 10px;">
        <el-input v-model="ModifiesFrom.SN"
                  type="text"
                  @input="searchClick"
                  style="width:200px"
                  clearable
                  :placeholder="$t('CollectProducts._62')" />
        <el-button type="primary"
                   icon="el-icon-search"
                   @click.prevent="searchClick">{{ $t("publics.btn.search") }}</el-button>
         <el-button type="danger"
                   @click.prevent="handelDeleteAll">{{ $t("publics.btn.deleteAll") }}</el-button>
      </div>
      <div class="AutomaticPackaging-table">
        <vxe-table ref="xTable"
                   ...
                   :seq-config="{startIndex: (ModifiesFrom.Page - 1) * ModifiesFrom.Limit}"
          ...
          <vxe-table-column min-width="200"
                            ...
                            :edit-render="{ name: 'input' }"
                            align="center" />
          <vxe-table-column :title="$t('MesQualityItems._27')"
                            min-width="180"
                            align="center"
                            fixed="right">
            <template v-slot="{ row }">
              <el-button type="primary"
                         ...
                         v-if="$btnList.indexOf('MesQualityItemsSaveData') !== -1">{{ $t('MesQualityItems._28') }}</el-button>
              <el-button type="danger"
                         @click="removeClick(row, row.$index)"
                         v-if="$btnList.indexOf('MesQualityItemsDelete') !== -1">{{ $t('MesQualityItems._29') }}</el-button>
            </template>
          </vxe-table-column>
        </vxe-table>
      </div>
      <el-pagination style="margin-top: 15px;"
                     :current-page="ModifiesFrom.Page"
                     :page-size="ModifiesFrom.Limit"
                     :page-sizes="[15, 25, 35, 45]"
                     layout="total, sizes, prev, pager, next, jumper"
                     :total="Modifiestotal"
    ...
    <el-dialog class="my-dialog"
               v-dialogDrag
               :title="$t('CollectProducts._63')"
               :visible.sync="dialogVisible"
               width="40%"
               :close-on-click-modal="false">
      <el-form ref="DeleditRef"
               label-width="100px"
               :model="Deledit">
        <el-form-item :label="$t('CollectProducts._64')+'：'">
          <el-input v-model="Deledit.OldSN"
                    type="text"
                    disabled
                    clearable
                    style="width:100%"
        ...
        <el-form-item :label="$t('CollectProducts._65')+'：'">
          <el-input v-model="Deledit.SN"
                    type="text"
                    style="width:100%"
                    placeholder=" " />
        </el-form-item>
  ...
  width="20%"
  :before-close="handleClose">
  <span>已扫码{{notifyNumber}}个</span>
  <span slot="footer" class="dialog-footer">
    <el-button @click="dialogVisibleNotify = false">取 消</el-button>
    <el-button type="primary" @click="dialogVisibleNotify = false">确 定</el-button>
  </span>
</el-dialog>
  </div>
</template>
...
export default {
  ...
  methods: {
    ...
    sonData (row) {
      debugger
      console.log(row)
      if (row !== null && row.DATA_TYPE === 3) {
        this.childVal.DATA = JSON.stringify(row)
        if (this.childVal.DATA) {
          this.InputData = this.childVal.DATA
          this.enterChange()
        }
      }
    },
    selectAllEvent ({ checked, records }) {
      this.annalTableSelect = records
    },
    selectChangeEvent ({ checked, records }) {
      this.annalTableSelect = records
    },
    async handelDeleteAll () {
      if (this.annalTableSelect && this.annalTableSelect.length <= 0) {
        this.$message({
          showClose: true,
          message: this.$t('CollectProducts._70'),
          type: 'warning'
        })
        return
      } else {
        var arr = []
        this.annalTableSelect.map((item) => {
          var obj = {}
          obj.LineId = this.StationFrom.OPERATION_LINE_ID
          obj.SiteId = this.StationID
          obj.CartonNo = item.CARTON_NO
          obj.SN = item.SN
          obj.OldSN = ''
          obj.UserName = this.userinfo.USER_NAME
          arr.push(obj)
        })
        console.log(arr)
        const res = await PackingDelBySN(arr)
        if (res.Result) {
          // 重新刷新一次
          var obj = {
            Date: this.NewEdition.CartonNo, // 数据（箱号 OR SN）
            CARTON_NO: '', // 卡通号（箱号）
            SN: '',
            Type: false,
            Page: 1,
          ...
          const ressn = await UpdateCartonNoBySN(obj) /// 箱号
          var datasn = ressn.Result
          if (datasn) {
            this.annalTable = datasn.PRODUCT || []
            this.Modifiestotal = ressn.TotalCount || 0
          }
          this.$notify({
          ...
          // this.getUpdateCarton()
        }
      }
      console.log(this.annalTableSelect)
    },
    // 获取首页预警信息数据
    ...
    async enterChange (row) {
      ...
      if (!this.DefinedQty) {
        this.$message({
          showClose: true,
          message: this.$t('CollectProducts._48'),
          type: 'warning'
        })
        return
      }
      console.log(this.DefinedQty)
      ...
      if (data) {
        ...
        if (this.ResultRun === 0) {
          ...
          this.notifySuccess++
          if (this.notifyFlag) {
            if (this.notifySuccess % parseInt(this.notifyNumber) === 0) {
        ...
        if (data.printer) {
          if (data.printer.isPrint) {
            this.printId = data.printer.printTaskId
    ...
    // SN修改
    ModifiesClick () {
      console.log(this.StationtoList, 'StationtoList')
      this.NewEdition.LineId = this.StationFrom.OPERATION_LINE_ID
      this.NewEdition.SiteId = Number(this.StationID)
      this.NewEdition.UserName = this.userinfo.USER_NAME

      if (this.Carton_NO) {
        this.NewEdition.CartonNo = this.Carton_NO
        this.NewEdition.OldSN = this.OldValue

        this.NewEdition.DEFINEDQTY = this.DefinedQty
        this.NewEdition.CURRENTQTY = this.CurrentQty
        // 获取表格
        this.Fillupform.CARTON_NO = this.CARTON_NO // 置满
        // this.ModifiesFrom.Date = this.OldValue
        this.UpdateCarton()
      }
      this.ModifiesoShow = true
    },
    // 修改输入数据回车
    DataChange (e) {
      if (!this.NewEdition.SN) {
        return
      }
      if (!this.NewEdition.CartonNo) {
        this.ModifiesFrom.Date = this.NewEdition.SN
        this.NewOldSN = this.NewEdition.SN // 删除使用旧SN查询
        this.UpdateCarton()
      } else {
        // 新增SN
        this.SavePackingDo()
    ...
    // 新增SN
    async SavePackingDo () {
      this.SiteObj.carton.Carton_NO = this.NewEdition.CartonNo
      this.SiteObj.carton.DefinedQty = this.NewEdition.DEFINEDQTY // Spenan
      this.SiteObj.data = this.NewEdition.SN
      const res = await PackingDoProcess(this.SiteObj) /// 箱号
      if (res.Result) {
        this.$notify({
          title: this.$t('cccn._24'),
          message: this.$t('CollectProducts._66'),
          type: 'success',
          duration: 2000
        })
        this.UpdateCarton()
    ...
    // 获取sn信息
    async UpdateCarton () {
      if (this.ModifiesFrom.SN) {
        this.ModifiesFrom.CARTON_NO = this.NewEdition.CartonNo
      } else {
        this.ModifiesFrom.CARTON_NO = undefined
      }
      const res = await UpdateCartonNoBySN(this.ModifiesFrom) /// 箱号
      var data = res.Result
      if (data) {
        this.NewEdition.CartonNo = data.CARTON_NO // 卡通号（箱号）
        this.NewEdition.CURRENTQTY = data.CURRENTQTY // 已刷数量
        this.NewEdition.DEFINEDQTY = data.DEFINEDQTY // 容量
        this.ModifiesFrom.Date = this.NewEdition.SN
        this.Fillupform.CARTON_NO = data.CARTON_NO// 置满
        this.annalTable = data.PRODUCT || []
        this.Modifiestotal = res.TotalCount || 0
      }
      // 报错处理
      if (res.warnResult) {
        this.annalTable = []
        this.Modifiestotal = 0
        // console.log(res.warnResult, 'res.warnResult')
      }
    },
    async getUpdateCarton () {
      if (this.ModifiesFrom.SN) {
        this.ModifiesFrom.CARTON_NO = this.NewEdition.CartonNo
      } else {
        // this.ModifiesFrom.CARTON_NO = undefined
    ...
    // 搜索
    searchClick () {
      this.ModifiesFrom.Type = true
      this.ModifiesFrom.Page = 1
      this.getUpdateCarton()
    },
    // 分页
    handleSizeChange (val) {
      this.ModifiesFrom.Limit = val
      this.getUpdateCarton()
    },
    handleCurrentChange (val) {
      this.ModifiesFrom.Page = val
      this.getUpdateCarton()
    },
    // 清空
    Clear () {
      ...
      this.annalTable = []
      this.NewEdition = {
        LineId: this.StationFrom.OPERATION_LINE_ID,
    ...
    // 编辑
    editRow (row) {
      this.Deledit.LineId = this.StationFrom.OPERATION_LINE_ID
      this.Deledit.SiteId = this.StationID
      this.Deledit.CartonNo = row.CARTON_NO
      this.Deledit.SN = ''
      this.Deledit.OldSN = row.SN
      this.Deledit.UserName = this.userinfo.USER_NAME

      this.dialogVisible = true
    },
    // 编辑提交
    async DeleditsSub () {
      if (this.Deledit.SN === '' || this.Deledit.SN === null) {
        this.$message.error(this.$t('CollectProducts._67'))
        return
      }
      // 获取线体对象
      // let sfcsOperationLines = {}
      // this.LineList.map(v => {
      //   if (this.Deledit.LineId === v.Id) {
      //     sfcsOperationLines = v
      //   }
      // })
      // 获取站点对象
      // const sfcsOperationSites = this.StationtoList.map(v => {
      //   if (this.Deledit.SiteId === v.ID) {
      //     return v
      //   }
      // })
      // const carton = { Carton_NO: this.Deledit.CartonNo }
      // const temp = {
      //   data: this.Deledit.SN,
      //   sn: this.Deledit.OldSN,
      //   sfcsOperationLines,
      //   sfcsOperationSites: sfcsOperationSites[0],
      //   carton,
      //   wono: this.ProductData.Wo_No
      // }
      this.SiteObj.data = this.Deledit.SN
      this.SiteObj.sn = this.Deledit.OldSN
      this.SiteObj.carton.Carton_NO = this.Deledit.CartonNo
      this.SiteObj.carton.DefinedQty = this.NewEdition.DEFINEDQTY
      this.SiteObj.woNo = this.ProductData.Wo_No
      console.log(this.SiteObj, 'Spenan')
      const res = await PackingModifyBySN(this.SiteObj)
      if (res.Result) {
        this.UpdateCarton()
        this.$notify({
          title: this.$t('publics.tips.tips'),
    ...
    removeClick (row) {
      ...
      this.Deledit.SiteId = this.StationID
      this.Deledit.CartonNo = row.CARTON_NO
      this.Deledit.SN = row.SN
      this.Deledit.OldSN = ''
      this.Deledit.UserName = this.userinfo.USER_NAME
      var arr = []
      arr.push(this.Deledit)
      if (row) {
        this.$confirm(
...
.AutomaticPackaging {
  ...
  .AutomaticPackaging-table {
    height: calc(100vh - 460px);
  }
}
.updateAuto ::v-deep .el-dialog{
  margin-top: 2vh  !important;
  margin-bottom: 0px  !important;
}
.changText ::v-deep .el-input__inner{
  color: red;
  font-weight: bold;
  font-size: 18px;
}
