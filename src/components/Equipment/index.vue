  ...
  <div class="equipment">
    ...
    >
      ...
      >
        <el-row :gutter="20">
          <el-col :span="12">
            <el-form-item :label="$t('设备类型：')" prop="MachineDevType">
              <el-select
                v-model="SetForm.MachineDevType"
                style="width: 100%"
                @change="MachineDevChange"
                placeholder="请选择设备类型"
              >
                <el-option
                  v-for="(item, index) in MachineDevType"
                  :key="index"
                  :label="item.CHINESE"
          ...
          <el-col :span="12">
            <el-form-item :label="$t('连接方式：')" prop="LinkeType">
              <el-select
                v-model="SetForm.LinkeType"
                style="width: 100%"
                @change="ConnCtion"
                placeholder="请选择连接方式"
              >
                <el-option
                  v-for="(itme, index) in LinkeType"
                  :label="itme.MEANING"
      ...
      <el-tabs type="border-card" v-model="activeName" @tab-click="handleClick">
        <el-tab-pane
          v-for="(itme, index) in LinkeType2"
          :label="itme.MEANING"
          :name="String(itme.LOOKUP_CODE)"
          :key="index"
        >
          <span slot="label"
            ><i :class="itme.ICON"></i> {{ itme.MEANING }}</span
          >
          <el-form
            ref="SetModl"
            label-width="100px"
          ...
          >
            ...
            <el-row>
              <el-col :span="24">
                <el-form-item :label="$t('标准接口：')" v-if="activeName == 4">
                  <el-checkbox-group
                    @change="NormintChan"
                    v-model="SetForm2.key41"
                  >
                    <el-checkbox true-label="Y" false-label="N"></el-checkbox>
                  </el-checkbox-group>
                </el-form-item>
              </el-col>
            </el-row>
            <el-row :gutter="20">
              <el-col :span="16">
                <el-form-item :label="$t('URL1：')" v-if="activeName == 4">
                  <el-input
                    v-model="SetForm2.key42"
                    clearable
                    type="text"
              ...
              <el-col :span="8">
                <el-form-item :label="$t('请求方式1：')" v-if="activeName == 4">
                  <el-select
                    v-model="SetForm2.key43"
                    style="width: 100%"
                    clearable
                    :disabled="NormalStart"
                    placeholder="请选择请求方式"
            ...
            <el-row :gutter="20">
              <el-col :span="16">
                <el-form-item :label="$t('URL2：')" v-if="activeName == 4">
                  <el-input
                    v-model="SetForm2.key44"
                    :disabled="NormalStart"
                    clearable
                    type="text"
                    placeholder="请输入URL2"
              ...
              <el-col :span="8">
                <el-form-item :label="$t('请求方式2：')" v-if="activeName == 4">
                  <el-select
                    v-model="SetForm2.key45"
                    clearable
                    :disabled="NormalStart"
                    style="width: 100%"
                    placeholder="请选择请求方式"
            ...
            <el-row :gutter="20">
              <el-col :span="16"
                ><el-form-item :label="$t('URL3：')" v-if="activeName == 4">
                  <el-input
                    v-model="SetForm2.key46"
                    clearable
                    :disabled="NormalStart"
                    type="text"
                    placeholder="请输入URL3"
            ...
            <el-row :gutter="20">
              <el-col :span="16">
                <el-form-item :label="$t('URL4：')" v-if="activeName == 4">
                  <el-input
                    v-model="SetForm2.key48"
                    clearable
                    :disabled="NormalStart"
                    type="text"
                    placeholder="请输入URL4"
            ...
            <el-row :gutter="20">
              <el-col :span="16"
                ><el-form-item :label="$t('URL5：')" v-if="activeName == 4">
                  <el-input
                    v-model="SetForm2.key50"
                    clearable
                    :disabled="NormalStart"
                    type="text"
                    placeholder="请输入URL5"
      ...
      <div slot="footer" style="text-align: center">
        ...
        <el-button type="primary" @click="SetOKClick">
          {{ $t("_kanban._5") }}
        </el-button>
      </div>
    </el-dialog>
...
export default {
  ...
  data () {
    return {
      LinkeType: [],
      LinkeType2: [
        { LOOKUP_CODE: 1, ICON: 'el-icon-coin', MEANING: '数据库' },
        { LOOKUP_CODE: 2, ICON: 'el-icon-files', MEANING: '文件' },
        { LOOKUP_CODE: 3, ICON: 'fa fa-usb', MEANING: 'COM' },
        { LOOKUP_CODE: 4, ICON: 'el-icon-connection', MEANING: 'WEBAPI' }
      ],
      MachineDevType: [],
      SetInfo: {
        MachineDevType: '', // 设备
  ...
  methods: {
    ...
    async SetOKClick () {
      ...
      } else if (this.SetForm.LinkeType === 4) {
        Itembox = [
          {
            Key: 41,
      ...
      this.$refs.SetModl[0].validate(
        async (valid, errInfo) => {
          if (valid) {
            ...
            if (res.Result) {
              this.GetMachineDetailInfo(obj.SiteID)
              // this.SetForm.DATA_TYPE = 1
              // this.websocketonopen()
              this.$notify({
    ...
    createWebSocket () {
      if (typeof WebSocket !== 'undefined') {
        ...
        this.$message.error(this.$t('当前浏览器不支持WebSocket！'))
        return true
      }
    },
    // 数据接收
    ...
    websocketonopen: function () {
      ...
      this.configData = {
        ...
        'DATA_TYPE': this.SetForm.DATA_TYPE,
        'LinkeType': this.SetForm.LinkeType,
        'MACHINE_TYPE': this.SetForm.MachineDevType,
        'SITE_ID': this.SetForm.SITE_ID,
        'SN': this.SetForm.SN,
        'USERNAME': this.SetForm.USERNAME,
        'WO_NO': this.SetForm.WO_NO
      }
      // 站点不相等换了站点重新断开连接
      ...
      // 数据发送
      console.log(this.configData, '发送数据')
      var Data = JSON.stringify(this.configData)
      this.StoreSiteId = this.configData.SITE_ID
      this.websock.send(Data)
    },
    ...
    websocketclose2 () {
      ...
      this.$confirm(this.$t('DockingStation._14'), this.$t('publics.tips.tips'), {
        ...
        .then(() => {
          ...
          // 数据发送
          console.log(this.configData, '发送关闭数据')
          var Data = JSON.stringify(this.configData)
          this.websock.send(Data)
          this.websocketclose()
        })
