          ...
          <el-tab-pane :label="$t('MesQualityInfo._108')"
            ...
            <div class="check-table">
              <vxe-table ref="xTable2"
                         ...
                         border
                         resizable
                        :tree-config="{lazy: true,children: 'ReplaceList',expandAll: true}"
                         :edit-config="{
                  trigger: 'click',
                  mode: 'row',
                  showStatus: true,
        ...
        <el-card class="box-card">
          <div slot="header"
            ...
            <el-button style="float: right; padding: 3px 0"
                       type="text"
                       @click="search_but">{{ $t("MesProductionPreMst._20") }}</el-button>
...
export default {
  ...
  created () {
    this.getInd()
    this.getList()
    this.getAddOrModifyBomDetail() // Spenan测试
    // this.getOrganize()
  },
  methods: {
    editClick (e) {
    ...
    async GetDetailBOM (id) {
      const res = await GetDetailBOMData(id)
      if (res.Result) {
