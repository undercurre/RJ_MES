<template>
  <d2-container>
    <template slot="header">
      <custom-container-header exportMehodsName="exportData" importBtnName="SmtResourceCategoryImport" exportBtnName="SmtResourceCategoryExport" exportTplName="SmtResourceCategoryExportTpl">
        <el-select
        ...
        >
          <el-option v-for="item in maneList" :key="item.ID" :label="item.NAME" :value="item.ID" />
        </el-select>&nbsp;
        <el-input
        ...
        />&nbsp;
        <el-input style="width:200px" v-model="formData.BRAND" :placeholder="'请输入辅料品牌'" @keyup.enter.native="searchClick" clearable></el-input>
        <el-input style="width:200px" v-model="formData.CATEGORY_MODEL" :placeholder="'请输入辅料规格'" @keyup.enter.native="searchClick" clearable></el-input>
        <el-button
          type="primary"
          icon="el-icon-search"
          @click.prevent="searchClick"
        >{{ $t('as_src.search') }}</el-button>
        <el-button
          type="success"
          icon="el-icon-plus"
          v-if="$btnList.indexOf('SmtResourceCategoryAdd') !== -1"
          @click.prevent="addClick(null)"
        >{{ $t('as_src.add') }}</el-button>
        <el-button
          type="primary"
          icon="el-icon-check"
    ...
    <div class="table-container">
      ...
      >
        ...
        <vxe-table-column :title="$t('as_src.tb_og')" min-width="120" align="center" fixed="right">
          <template v-slot="{ row }">
            <el-button type="danger" @click="removeClick(row, row.$index)" v-if="$btnList.indexOf('SmtResourceCategoryRemove') !== -1">{{ $t('as_src.tb_rm') }}</el-button>
          </template>
        </vxe-table-column>
      </vxe-table>
    </div>
