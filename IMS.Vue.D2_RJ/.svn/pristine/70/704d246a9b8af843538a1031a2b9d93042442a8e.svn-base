            ...
            <div class="block-center">
              <div class="kanban-wo-yield-panel kanban-panel-bg">
                <div class="block-center-content">
                  <div style="font-size: 20px;">{{$t("DockingStation._1")}}</div>
                  <el-form ref="form"
                           :show-message="false"
                           class="collstat-form"
                           label-width="80px"
              ...
              <div class="kanban-hour-yield-panel kanban-panel-bg"
                ...
                <div id="swiper1"
                  ...
                  <div class="block-center-content">
                    ...
                    <div class="center-centent-testmsg">
                      {{TEST_MSG}}
                    </div>
                  </div>
                </div>
...
export default {
  ...
  created () {
    ...
    if (typeof WebSocket !== 'undefined') {
      var websock = new WebSocket('ws://localhost:42738')
      // WebSocket成功处理
      websock.onopen = function (evt) {
        if (evt.type === 'open') {
          ...
          tant.getAddOrModify()
          if (!tant.StationID) {
            tant.dialogStation = true
          }
          if (!tant.WoNoBer) {
            tant.dialogWoNo = true
          }
        }
      }
      // WebSocket失败处理
      websock.onerror = function (evt) {
        if (evt.type === 'error') {
          tant.$message.error(tant.$t('DockingStation._10'))
        }
      }
    } else {
  ...
  methods: {
    // 子里接收的数据
    sonData (row) {
      console.log(row, '子里接收的数据')
      this.TEST_MSG = row.TEST_MSG || ''
    ...
    async enterChange (row) {
      if (!this.SetSTATUS) {
        this.$message({
          showClose: true,
          message: this.$t('DockingStation._11'),
          type: 'warning'
        })
        return
      }
      ...
      this.childVal.DATA = this.InputData// 设备对接
      this.OldValue = this.InputData
      this.childVal.SN = this.OldValue// 设置
      this.InputData = ''
      this.childVal.DATA_TYPE = 3// 设备对接
      this.$refs.RefChildVal.FatherClick(this.childVal)
    },
