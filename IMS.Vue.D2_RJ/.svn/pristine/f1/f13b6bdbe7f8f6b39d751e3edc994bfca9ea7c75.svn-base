<template>
  <d2-container class="SfcsProcessBad">
    ...
    <el-row style="margin-top: 0px">
      <el-tabs
        ...
        ref="tableTransfer"
      >
        <el-tab-pane name="BE_DAY" label="制程不良时段-日">
          <div
        ...
        <el-tab-pane name="BE_WEEK" label="制程不良时段-周">
          <div
        ...
        <el-tab-pane name="BE_MONTH" label="制程不良时段-月">
          <div
...
import {
  ...
  LineProductionCapacityByFail,
  SiteFailDetailsByLine
...
export default {
  ...
  methods: {
    async SiteFailDetailsByLine (obj) {
      const res = await SiteFailDetailsByLine(obj)
      if (res.Result) {
    ...
    handelStatisticsList () {
      ...
      this.LineProductionCapacityByFail()
      console.log(this.echartForm)
    },
    async LineProductionCapacityByFail () {
      const res = await LineProductionCapacityByFail(this.echartForm)
      if (res.Result) {
    ...
    async getHistogram (data, typeID) {
      ...
      if (data.length !== 0) {
        ...
        Object.keys(data).forEach((key, index) => {
          ...
          data[key].map((item) => {
            obj.data.push(item.FAIL_RATE)

            if (flag <= data[key].length - 1) {
              this.exportData[flag][key] = item.FAIL_RATE
            }
            flag++
          })
          flag = 0
