<template>
  <d2-container>
    ...
    <div class="SmtStencilConfig-center">
      <vxe-table ref="xTable"
        ...
        <vxe-table-column :title="$t('publics.field.operate')"
                          ...
                          fixed="right">
          <template v-slot="{ row }">
            <el-button v-if="$btnList.indexOf('GetStencilResourceList') !== -1"
                       @click.stop="openAnnexModel(row)">{{ $t('SmtStencilConfig._17') }}</el-button>
            <el-button type="primary"
          ...
          <el-row>
            <el-col :span="12">
              <el-form-item :label="$t('SmtFeederConfig2._6')"
                            prop="LINE_NAME">
                <el-input v-model="modifyForm.ATTRIBUTE4" />
              </el-form-item>
            </el-col>
            <el-col :span="12">
    ...
    <!-- 附件列表 -->
    <el-dialog class="currentAnnex"
               :title="$t('SmtStencilConfig._17')"
               :visible.sync="annexModel"
               width="60%"
               top="5vh"
               :close-on-click-modal="false">
      <div style="display: flex;padding-bottom: 10px">
        <el-input clearable
                  @input="AnnexSearch"
                  v-model="annexQuery.RESOURCE_NAME"
                  :placeholder="$t('SmtStencilConfig._18')"></el-input>
        <el-button icon="el-icon-search"
                   @click="AnnexSearch"
                   type="primary">{{ $t('SmtStencilConfig._19') }}</el-button>
        <el-upload v-if="$btnList.indexOf('UploadResource') !== -1"
                   ref="reFpslcFile"
                   style="margin-left: 10px"
                   :action="'#'"
                   :http-request="(event) => handleRequestFile(event)"
                   :show-file-list="false"
                   :limit="20"
                   accept="txt">
          <el-button type="success"
                     icon="el-icon-upload"> {{ $t('SmtStencilConfig._20') }} </el-button>
        </el-upload>
      </div>
      <div style="height: 400px">
        <vxe-table ref="annexTable"
                   border
                   resizable
                   height="100%"
                   size="medium"
          ...
          <vxe-table-column min-width="150"
                            field="RESOURCE_NAME"
                            :title="$t('SmtStencilConfig._21')"
                            :edit-render="{ name: '$input', props: { readonly: true } }" />
          <vxe-table-column min-width="150"
                            field="UPLOAD_OPER"
                            :title="$t('SmtStencilConfig._22')"
                            :edit-render="{ name: '$input', props: { readonly: true } }" />
          <vxe-table-column min-width="200"
                            field="CREATE_TIME"
                            :title="$t('SmtStencilConfig._23')"
                            :edit-render="{ name: '$input', props: { readonly: true } }" />
          <vxe-table-column :title="$t('as_src.tb_og')"
            ...
            <template v-slot="{ row }">
              <el-button type="success"
                         @click="Preview(row, row.$index)">
                {{ $t('se_cc.preview') }}
              </el-button>
              <el-button type="primary"
                         v-if="$btnList.indexOf('GetStencilResourceInfo') !== -1"
                         @click="downloadAnnex(row, row.$index)">
                {{ $t('SmtStencilConfig._26') }}
              </el-button>
            </template>
          </vxe-table-column>
        </vxe-table>
...
import {
  // GetStencilResourceInfo,
  GetStencilResourceList,
  DownloadResource
} from '@/api/SmtStencilConfig'
...
export default {
    ...
    return {
      validRules: {
        STENCIL_NO: [
          {
            required: true,
            message: this.$t('SmtStencilConfig._13')
          }
        ]
      },
      currentRowIndex: -1,
      ...
      ProductPage: 0,
      uploadFileUrl:
        process.env.VUE_APP_API + 'SmtStencilConfig/UploadResource',
      annexModel: false,
      annexList: [],
      annexLoading: false,
      annexTotal: 0,
  ...
  methods: {
    ...
    async handleRequestFile (data) {
      // 上传钢网附件
      if (!this.annexQuery.STENCIL_ID) {
        this.$message.error(this.$t('SmtStencilConfig._24'))
        return false
      }
      console.log(data)
      let formdata = new FormData()
      formdata.append('file', data.file)
      formdata.append('userName', this.username)
      formdata.append('stencil_id', this.annexQuery.STENCIL_ID)
      const infoNotify = this.$notify.info({
        title: this.$t('SmtProducePlan._21'),
        dangerouslyUseHTMLString: true,
        duration: 0,
      ...
      const res = await request
        .post(this.uploadFileUrl, formdata, {
          ...
          params: {
            ...
            stencil_id: this.annexQuery.STENCIL_ID
          },
          timeout: 60000 * 5 // 请求超时时间
        })
        .catch((_) => {
      ...
      if (res.Result) {
        ...
        this.apiGetStencilResourceList()
      } else {
        this.$notify.error({
          title: this.$t('SmtProducePlan._23'),
          message: this.$t('SmtProducePlan._24')
    ...
    AnnexSearch () {
      this.annexQuery.Page = 1
      this.apiGetStencilResourceList()
    },
    async apiGetStencilResourceList () {
      this.annexLoading = true
      const res = await GetStencilResourceList(this.annexQuery).catch(_ => {
        this.annexLoading = false
      })
      this.annexLoading = false
      if (res.Result) {
        const data = res.Result || '[]'
        this.annexList = JSON.parse(data) || []
        this.annexTotal = res.TotalCount || 0
      }
    },
    async downloadAnnex (row, index) { // 下载附件
      // eslint-disable-next-line camelcase
      const resource_id = row.ID || 0
      // eslint-disable-next-line camelcase
      if (!resource_id) {
        this.$message.error(this.$t('SmtStencilConfig._25'))
        return false
      }
      // const res = await GetStencilResourceInfo(resource_id)
      const res = await DownloadResource(resource_id)
      if (res.Result) {
        const RESOURCE_DATA = res.Result
        if (!RESOURCE_DATA) {
          this.$message.error(this.$t('SmtStencilConfig._25'))
          return false
        }
        downloadjs(RESOURCE_DATA, row.RESOURCE_NAME || `${this.$t('SmtStencilConfig._17')}${row.ID || 0}`)
      }
    },
    Preview (row) {
      this.PreviewImg = row.RESOURCE_URL
