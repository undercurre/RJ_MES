<template>
  <custom-container>
    ...
    <!-- 上传抽屉modal -->
    <el-drawer :title="$t('SmtPlacementHeader._115')"
               :visible.sync="drawer"
               :model="false"
               direction="ltr"
               size="30%">
      <div style="padding: 0 20px">
        <el-form :model="uploadForm"
          ...
          <el-form-item :label="$t('SmtPlacementHeader._80')"
            ...
            <!-- select start -->
            <!-- <el-select v-model="uploadForm.Part_NO"
                   filterable
                   clearable
                   allow-create
                   placeholder=" "
          ...
          <div style="width: 600px;display: flex;padding: 0 20px 0 10px;position: sticky;top: 6px;background: #fff;z-index: 90">
            <el-input v-model="listQuery.PART_NO"
                      @keyup.enter.native="getSmtStencilPartList"
                      placeholder=" "></el-input>
            <el-button type="primary"
                       icon="el-icon-search"
                       @click.prevent="getSmtStencilPartList">{{ $t('publics.btn.search') }}</el-button>
          </div>
          <el-option v-for="item in SmtStencilPartList"
                     :key="item.ID"
                     :label="item.PART_NO"
                     :value="item.PART_NO" >
                      <span style="float: left">{{ item.PART_NO }}</span>
          ...
          <div style="width: 600px;position: sticky;bottom: 6px;background: #fff;z-index: 90;padding-left: 15px">
            <el-pagination :pager-count="5"
                           ...
                           :total="SmtStencilPartTotalCount"
        ...
        <!-- select end -->
          ...
          <el-form-item :label="$t('SmtPlacementHeader._118')">
            <el-select v-model="currentLineId" style="width: 100%" placeholder=" " @change="handleChangeLineId">
              <el-option v-for="item in lineList" :key="item.LINEID" :label="item.LINE_NAME" :value="item.LINEID"></el-option>
            </el-select>
          </el-form-item>
          ...
          <el-form-item :label="$t('SmtPlacementHeader._82')"
            ...
            <el-checkbox-group v-model="choosenSMTStationList"
              ...
              <el-checkbox :label="item.SMT_NAME"
                           v-for="item in SmtStation"
                           :key="item.STATIONID" ></el-checkbox>
            </el-checkbox-group>
          </el-form-item>
...
export default {
  ...
  data () {
    const checkPartNo = (rule, value, callback) => {
      if (!value) {
        callback(Error(this.$t('SmtPlacementHeader._100')))
      }
      callback()
    }
    return {
      ...
      actionUrl: `${process.env.VUE_APP_API}SmtPlacementHeader/ImportSiemensPlacementFile`,
      SmtStation: [],
      PcbSide: [],
      ...
      validRules: {
        ...
        // ],
        PART_NO: [{ required: true, validator: checkPartNo }],
        // REFDESIGNATOR: [
      ...
      SmtStencilPartList: [],
      SmtStencilPartTotalCount: 0,
      lineList: [],
      currentLineId: 0,
      originSmtStation: [],
      // !- 开关 用于做文件缓存队列次数 目前只是控制 1 次和 无限次
      count: 1
    }
  },
  mounted () {
    ...
    this.initPage()
    // this.getSmtStencilPartList()
  },
  beforeDestroy () {
    clearTimeout(this.timer)
    this.SET_SIMENS_FLAG(false)
  ...
  methods: {
    ...
    openUploadModal () {
      ...
      this.choosenSMTStationList = this.SmtStation.map(i => i.SMT_NAME) || []
      let tmp = []
      this.SmtStation.map((item) => {
        if (this.choosenSMTStationList.indexOf(item.SMT_NAME) !== -1) {
          tmp.push(item.STATIONID)
        }
      })
    ...
    handleUploadRequest (data) {
      ...
      formdata.append('StationIdArrary', this.uploadForm.Stations)
      ...
      request
        ...
        .then((res) => {
          ...
          this.TitleList = [{ field: 'SLOT', fileName: '主料槽' }, { field: 'PART_NO', fileName: '料号信息' }, { field: 'REFDESIGNATOR', fileName: '位号信息' }, { field: 'UNITQTY', fileName: '用量' }, { field: 'FEEDER_TYPE', fileName: '飞达规格' }, { field: 'STAGE', fileName: '模组' }, { field: 'SUBSLOT', fileName: '子料槽' }]
          // TODO 新修改 -----------
          const LinePlacementArrary = (AIRIPlacement?.DataList || {})?.LinePlacementArrary || []
          this.DataList = []
          LinePlacementArrary.map(i => {
            const StationID = i.StationID || 0
            const PlacementDetailArrary = i.PlacementDetailArrary || []
            PlacementDetailArrary.map(j => {
              j.StationID = StationID
              return j
            })
            this.DataList = this.DataList.concat(PlacementDetailArrary)
          })
          // TODO 新修改 end ----------
    ...
    async saveData () {
      ...
      this.fileDataCache.map((item, index) => {
        ...
        commitData.push({
          ...
          DataList: fullData//item.DataList
        })
      })
      const res = await SiemensPlacementSaveData({
    ...
    async initPage () {
      const res = await ImportAIPlacementView(
        this.machineList['SIEMENS'] || ''
      )
      if (res.Result) {
        const data = res.Result
        const SmtStation = data.SmtStation
        // TODO 区分出线体和机台
        this.lineList = []
        // const lineMap = [...new Set(SmtStation.map(i => i.LINEID))]
        const lineMap = {}
        SmtStation.map(i => {
          lineMap[i.LINEID] = {
            LINEID: i.LINEID,
        ...
        this.originSmtStation = SmtStation
        this.lineList = Object.values(lineMap).map(i => i)
        this.currentLineId = this.lineList[0]?.LINEID || 0

        this.SmtStation = SmtStation.filter(i => i.LINEID === this.currentLineId)
        // TODO 默认全选
        this.choosenSMTStationList = this.SmtStation.map(i => i.SMT_NAME)
        let tmp = []
        this.SmtStation.map((item) => {
    ...
    handleChangeLineId (e) {
      this.SmtStation = this.originSmtStation.filter(i => i.LINEID === this.currentLineId)
      // TODO 默认全选
      this.choosenSMTStationList = this.SmtStation.map(i => i.SMT_NAME)
      let tmp = []
      this.SmtStation.map((item) => {
    ...
    writeToFileCache () {
      const tmp = {
        ...
        MultiNo: this.uploadForm.MultiNo || 0
      }
      let flag = true
      let _index = -1
      try {
        this.fileDataCache.map((item, index) => {
          ...
          if (item.Part_NO === tmp.Part_NO && item.PCB_SIDE && tmp.PCB_SIDE && JSON.stringify(item.Stations) === JSON.stringify(tmp.Stations)) {
            // TODO 覆盖操作
            item = tmp
            _index = index
            flag = false
    ...
    _bomCompareExecute () {
      ...
      this.faildDetails = []
      this.fileDataCache.map(async (_item, _index) => {
        const ref = 'xTable' + (_index + 7)
    ...
    // async getSmtStencilPartList () {
    //   this.partNoListLoading = true
    //   const res = await API.LoadData(this.listQuery).catch(_ => {
    //     this.partNoListLoading = false
    //   })
    //   this.partNoListLoading = false
    //   this.SmtStencilPartList = res.Result || []
    //   this.SmtStencilPartTotalCount = res.TotalCount || 0
    // },
    listQuerySizeChange (size) {
