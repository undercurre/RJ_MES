<template>
  <custom-container>
    ...
    <!-- 上传抽屉modal -->
    <el-drawer :title="$t('SmtPlacementHeader._114')"
               :visible.sync="drawer"
               :modal="false"
               direction="ltr"
               size="30%">
...
export default {
  ...
  data () {
    return {
      ...
      actionUrl: `${process.env.VUE_APP_API}SmtPlacementHeader/ImportRIPlacementFile`,
      SmtStation: [],
      PcbSide: [],
  ...
  methods: {
    ...
    // BOM比对完之后提交保存入库操作
    // 取消await this.compareByBom() --chenhx
    async saveData () {
      ...
      const res = await AIPlacementSaveData({
        ...
        DataList: fullData//this.DataList
      })
      if (res.Result) {
        this.$notify.success({
          title: this.$t('SmtPlacementHeader._103'),
    ...
    // 获取视图
    async initPage () {
      const res = await ImportAIPlacementView(this.machineList['RI'] || '')
      if (res.Result) {
