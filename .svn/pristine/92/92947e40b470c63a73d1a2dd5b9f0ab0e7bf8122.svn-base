...
export default {
  ...
  methods: {
    ...
    cellClickReasonCode ({
      ...
      this.submitForm.REASON_CODE = row.REASON_CODE
    },
    ...
    async GetSnDataList () {
      const res = await GetSnDataList({
        ...this.SnListQuery,
        OPER_ID: this.currentSite.OPER_ID,
      })
    ...
    handleMakeSureReasonCode () {
      ...
      this.submitForm.CHINESE_DESCRIPTION = this.chosenReasonCode.CHINESE_DESCRIPTION
